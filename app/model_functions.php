...
include_once 'utility_functions.php';
  ...
  function setHousehold($params) {
    ...
    if ($row_count) {
      $st1 = $db->prepare("UPDATE contactperson SET 
      ...
      $st1 = $db->prepare("INSERT INTO contactperson SET 
        First_Name=:First_Name, 
        Last_Name=:Last_Name, 
        Address_1=:Address_1, 
        Address_2=:Address_2,
    ...
    try { 
      ...
      $st2 = $db->prepare("SELECT * FROM contactperson WHERE Email1=?");
      $st2->execute(array($params['Email1']));
      $rows = $st2->fetchAll(PDO::FETCH_ASSOC);
      $st2->closeCursor();

      if ($st2->rowCount()) {
        $st3 = $db->prepare("UPDATE account SET
          Parent_or_Guardian_1_Contact_Information_Id=:ContactPerson_Id
          WHERE Account_Id=:Account_Id
        ");

        try { 
          $st3->execute(array(':ContactPerson_Id' => $rows[0]['ContactPerson_Id'], ':Account_Id' => $_SESSION["account_id"]));
        } catch(PDOExecption $e) { 
          print "Error!: " . $e->getMessage(); 
        }
      } 
  ...
  function insertStudent($data) {
    ...
    try { 
      $stmt->execute(array(':Account_Id' => $data['Account_Id'], ':First_Name' => $data['First_Name'], ':Middle_Name' => $data['Middle_Name'], ':Last_Name' => $data['Last_Name'], ':Grade_Id' => $data['Grade_Id'], ':Birth_Date' => $data['Birth_Date'], ':DiplomaType_Id' => $data['DiplomaType_Id']));
    } catch(PDOExecption $e) { 
      print "Error!: " . $e->getMessage(); 
    }
